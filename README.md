...
Double click an image, audio file, or video, press Enter, or choose `Preview` from its context menu to open a large preview window. Images load at their original resolution and fit within the window; audio and video use native playback and seek controls. Media streams through authenticated, version-specific URLs with byte-range support. Playback depends on the browser's supported formats and codecs; Vault does not transcode media. Unplayable files offer retry and download actions. The double-click download preference applies to files without previews.
