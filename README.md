...
By default `docker-compose.yml` runs `ghcr.io/willjallen/vault:latest`, binds the service to `127.0.0.1:8000`, uses header-based auth for a trusted reverse proxy, and mounts a single named volume at `/data`.
