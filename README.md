...
Downloads use the browser's native download manager by default. A single-request File System Access stream is available behind the `customDownloadStreamingEnabled` site setting, which defaults to `false`; it avoids range probes, parallel segments, workers, and whole-file browser buffers. Vault always presents one `Download` action: when the gate and browser API are available it opens the system file picker, otherwise it uses the native download manager. Browsers without that API show guidance explaining the browser-wide download-location setting until the user selects `Do not show again`; that dismissal is stored in the user's synced Vault preferences.
