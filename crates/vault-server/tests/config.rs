...
fn dockerfile_runs_rust_server_with_single_data_volume_contract() {
    ...
    assert!(dockerfile.contains("COPY --from=assets --chown=vault:vault /build/app/static/dist"));
    ...
    assert!(dockerfile.contains("VAULT_STATIC_DIR=/app/app/static"));
...
fn generated_static_assets_are_ignored_build_output() {
    ...
    assert!(dockerignore.contains("app/static/dist/"));
    assert!(dockerignore.contains("target/"));
    assert!(gitignore.contains("app/static/dist/"));
}
